...
public:
	...
	bool Capture(const int& newX, const int& newY, bool& turn, sf::RenderWindow& window, std::vector<std::vector<Square>>& squares) override {
		...
		if (squares[newX][newY].x == x && squares[newX][newY].y < y) dir1 = true;
		if (squares[newX][newY].x < x && squares[newX][newY].y == y) dir2 = true;
		if (squares[newX][newY].x == x && squares[newX][newY].y > y) dir3 = true;
		if (squares[newX][newY].x > x && squares[newX][newY].y == y) dir4 = true;
		...
		if ((dir1 && squares[newX][newY].y >= dir1Y) ||
			(dir2 && squares[newX][newY].x >= dir2X) ||
			(dir3 && squares[newX][newY].y <= dir3Y) ||
			(dir4 && squares[newX][newY].x <= dir4X)) return true;
		else return false;
	}
};
...
public:
	...
	bool Capture(const int& newX, const int& newY, bool& turn, sf::RenderWindow& window, std::vector<std::vector<Square>>& squares) override {
		if (abs(newX - x) <= 1 && abs(newY - y) <= 1) return true;
		return false;
	}
};
